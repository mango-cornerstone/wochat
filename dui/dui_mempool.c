...
#define PG_SIZE_MAX               (~(size_t)0)
 /* Must be less than PG_SIZE_MAX */
#define MaxAllocHugeSize	(PG_SIZE_MAX / 2)

#define InvalidAllocSize	PG_SIZE_MAX
...
MemoryPoolContext AllocSetContextCreateInternal2(MemoryPoolContext parent, const char* name, Size minContextSize, Size initBlockSize, Size maxBlockSize)
{
...
char* pstrdup(MemoryPoolContext cxt, const char* in)
{
	if(cxt)
		return MemoryContextStrdup((MemoryContext)cxt, in);
 ...
 */
void* MemoryContextAllocExtended(MemoryPoolContext ctx, Size size, int flags)
{
	void* ret;
 ...
 */
void MemoryContextDelete(MemoryPoolContext cxt)
{
	MemoryContext context = (MemoryContext)cxt;
