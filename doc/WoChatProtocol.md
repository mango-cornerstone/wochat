...
用户发送的任何一条信息，不管是一段文本，还是图片，视频，或者其它文件，都可以理解是为具有n个字节的字节数组。 它必然有一个固定的SHA256哈希值。所以我们根据这32字节的哈希值就能够唯一性地确定用户发送的数据。 接收方收到消息后，可能需要给发送方一个确认，确认的方法是把收到的数据的SHA256哈希值发送回去。发送方收到后，就百分百确定对方收到了自己的数据，因为对方不可能凭空捏造出一个一模一样的哈希值。同样的，发送方发送以后后悔了，想撤销这个数据，他也是把这条消息的SHA256哈希值发送给接收方。当然，撤不撤销，接收方说了算。 对于数据操作的消息包，我们提供统一的格式，如下图所示：

![](x0002.svg) 

这种类型的消息包开始88个字节，依然是发送者和接收者的公钥的base64编码，各有44个字节。分隔符由竖线变成了#，表明这是一个操作消息包。后面是44个字节的base64编码的数据。解码后就是33个字节，其中第一个字节是对消息的操作类型，可以表示256种可能性，我们称之为Op字节(operation)。后面32个字节就是数据的SHA256哈希值，这个哈希值是使用主密钥Kp用AES256算法加密的。 我们取解密后的SHA256从左往右数第一个非零字节，和Op字节进行异或，算是对Op字节的加密。
